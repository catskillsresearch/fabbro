...
flowchart LR
  ...
  subgraph ML["Mathlib"]
    ...
    LAlgebraOrderRingUnbundledRat["Algebra.Order.Ring.Unbundled.Rat"]:::mathlib
  ...
  LDataSetFiniteBasic --> MWellOrder
  LDataRatDefs --> MWellOrder
  LOrderIntervalSetBasic --> MWellOrder
  LAlgebraOrderRingUnbundledRat --> MWellOrder
...
We define $S = \mathbb{Z} \times (\mathbb{Z} \setminus \{0\})$ and
$(a,b) \sim (c,d) \iff ad = bc$.

- **Reflexivity.** $ab = ba$ holds by commutativity of multiplication in $\mathbb{Z}$.
- **Symmetry.** $ad = bc$ implies $cb = da$, so $(c,d) \sim (a,b)$.
- **Transitivity.** If $ad = bc$ and $cf = de$, multiplying through yields
  $$(af)d = (ad)f = (bc)f = b(cf) = b(de) = (be)d.$$
  Since $d \ne 0$ and $\mathbb{Z}$ is an integral domain (has no zero divisors),
  $(af - be)d = 0$ implies $af = be$.
