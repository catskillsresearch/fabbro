...
<!-- lean-dep-figure -->
<!-- figure-caption: Lean module dependencies for the 21-373 solution. Amber box: Mathlib imports. Blue box: solution modules. -->
```mermaid
...
flowchart LR
  ...
  subgraph ML["Mathlib"]
    ...
    LDataFinsetLatticeFold["Data.Finset.Lattice.Fold"]:::mathlib
    LDataFintypeBasic["Data.Fintype.Basic"]:::mathlib
    LOrderAtoms["Order.Atoms"]:::mathlib
  ...
  subgraph SOL["Solution modules"]
    ...
    MAtomJoin["AtomJoin"]:::course
    MBooleanAction["BooleanAction"]:::course
    MIsBooleanAtom["IsBooleanAtom"]:::course
    MRepresentation["Representation"]:::course
    MCourse21373Numerical["Numerical"]:::course
  end

  LDataFinsetLatticeFold --> MAtomJoin
  LAlgebraBigOperatorsGroupFinsetBasic --> MAtomJoin
  MIsBooleanAtom --> MAtomJoin
  MAtomJoin --> MBooleanAction
  LOrderAtoms --> MIsBooleanAtom
  LOrderBooleanAlgebraBasic --> MIsBooleanAtom
  LDataFintypeBasic --> MRepresentation
  MAtomJoin --> MRepresentation
  LAlgebraBigOperatorsGroupFinsetBasic --> MCourse21373Numerical
```
<!-- /lean-dep-figure -->
...
### (a) Every element is a unique join of a subset of the atoms
...
### (b) Isomorphism with $\mathcal{P}(\{a_1,\dots,a_n\})$ and identification with $\{0,1\}^n$
...
### (c) Action of a finite group $G$ by automorphisms
